...
#        
    ...
    def addToBatch(self):
        if self.ui.batch_protein_A.text() != "" and self.ui.batch_protein_B.text() !="":
            ...
            pathAID = self.ui.batch_protein_A_ID.text()
            pathB = self.ui.batch_protein_B.text()
            pathBID = self.ui.batch_protein_B_ID.text()           
            if os.sep == "\\":
                pathA = pathA.replace("/","\\")
                pathB = pathB.replace("/","\\")
                pathBID = pathBID.replace("/","\\")            
                pathAID = pathAID.replace("/","\\")
                
            temp = "batch id: "+str(self.ui.batch_list.count()+1)+"\nprotA: "+pathA +"\nprotAID: "+pathAID + "\nprotB: "+ pathB+ "\nprotBID: "+pathBID
            QListWidgetItem(temp, self.ui.batch_list)
            self.ui.batch_protein_A.setText("")
            self.ui.batch_protein_A_ID.setText("")
            self.ui.batch_protein_B.setText("")
            self.ui.batch_protein_B_ID.setText("")
            self.checkEnabled()
    ...
    def selectView(self,view):
        if view==0:
            ...
            self.ui.quick_help.setText("In the \"Protein Input\" view, you can select protein (.pdb) files and chain ID to map to each other. After selection click on Add. Then select an output destination folder and filename convention.")
        if view==1:
            ...
            self.ui.quick_help.setText("In the \"Analysis\" view, you can view logged output from the protein mapping. In addition, you can use Contact's finedata 2d plot feature, as well as directly export the results to a spreadsheet.")
...
#        self.ui.tabWidget.addTab(analysisWidget,"test")
        if os.path.isfile(self.filename+".s"):
            ...
            with open(self.filename+".s","r") as fort18:
                for line in fort18:
                    counter+=1
                    if counter==7:
                        count_a = int(line[26:].strip())
                    if counter==8:
                        count_b = int(line[26:].strip())
                ...
                self.output(str(count_a)+" "+str(count_b))
    ...
    def graphClick(self):
        ...
        filterWidgetHi = self.ui.tabWidget.currentWidget().findChild(QWidget).findChild(QCheckBox,"graph_filter_hi")
        filterWidgetLow = self.ui.tabWidget.currentWidget().findChild(QWidget).findChild(QCheckBox,"graph_filter_low")
        filterAmountHi = None
        filterAmountLow = None
        if filterWidgetHi.isChecked():
            filterAmountHi = self.ui.tabWidget.currentWidget().findChild(QWidget).findChild(QSpinBox,"graph_filter_amount_hi").value()
        if filterWidgetLow.isChecked():
            filterAmountLow = self.ui.tabWidget.currentWidget().findChild(QWidget).findChild(QSpinBox,"graph_filter_amount_low").value()
        if self.ui.tabWidget.currentWidget().findChild(QWidget).findChild(QComboBox,"graph_data").currentIndex() == 0:
            self.showGraph(self.sepDistance,"Angstroms apart", filterAmountHi,filterAmountLow)
        elif self.ui.tabWidget.currentWidget().findChild(QWidget).findChild(QComboBox,"graph_data").currentIndex() == 1:
            self.showGraph(self.lennardJones,"P",filterAmountHi,filterAmountLow)
        elif self.ui.tabWidget.currentWidget().findChild(QWidget).findChild(QComboBox,"graph_data").currentIndex() == 2:
            self.showGraph(self.coulombicPotential,"P",filterAmountHi,filterAmountLow)
        end_time = time.time()
    ...
    #GRAPHING METHOD
    def showGraph(self, value, unitLabel, filterAmountHi, filterAmountLow):
        A = np.array(self.atomNumberA)
        B = np.array(self.atomNumberB)
        Z = np.copy(value)
        A = np.unique(A)
        B = np.unique(B)
        self.output(str(len(A))+ " "+str(len(B)))
        #Z = np.zeros((A.size,B.size))
        i = 0
        for x in range(0,A.size):
            for y in range(0,B.size):
                if filterAmountHi != None and Z[x,y] > filterAmountHi:
                    Z[x,y] = filterAmountHi
                if filterAmountLow != None and Z[x,y] < filterAmountLow:
                    Z[x,y] = filterAmountLow
        fig = plt.figure()
 ...
 #       lowBound = np.min(S)
 #       highBound = np.max(S)
        #specific range np.arange(lowBound,highBound,(highBound-lowBound)/10)
...
#            QMessageBox.information(self,"Large Mapping Detected","Large protein mapping detected. The mapping may take several minutes, during which time the program may appear to hang.",1)
        ...
        if os.path.isfile(filename):
            self.atomLabelA =list()
            self.atomLabelB = list()
            numA = []
            numB = []
            sepDistance = {}
            lennardJones = {}
            coulombicPotential = {}
            counter=0
            with open(filename,"r") as fort16:
                for line in fort16:
                    reader = line.split(',')
                    temp = reader[2].strip(' ') + ", " + reader[0].strip(' ') + "-" + reader[1].strip(' ')
                    indexA = int(reader[3].strip(' '))
                    if temp not in self.atomLabelA:
                        self.atomLabelA.append(temp)
                        numA.append(indexA)

                    temp = reader[6].strip(' ') + ", " + reader[4].strip(' ') + "-" + reader[5].strip(' ')
                    indexB = int(reader[7].strip(' '))
                    if temp not in self.atomLabelB:
                        self.atomLabelB.append(temp)
                        numB.append(indexB)

                    temp = reader[8]
                    sepDistance[indexA,indexB] = float(temp)
                    temp = reader[9]
                    lennardJones[indexA,indexB] = float(temp)
                    temp = reader[10]
                    coulombicPotential[indexA,indexB] = float(temp)
                    counter+=1

            self.sepDistance = np.empty([len(numA),len(numB)])
            self.lennardJones = np.empty([len(numA),len(numB)])
            self.coulombicPotential = np.empty([len(numA),len(numB)])
            ca=0
            for a in numA:
                cb=0
                for b in numB:
                    try:
                        self.sepDistance[ca,cb] = sepDistance[a,b]
                    except:
                        self.sepDistance[ca,cb] = 99
                    try:
                        self.lennardJones[ca,cb] = lennardJones[a,b]
                    except:
                        self.lennardJones[ca,cb] = 99
                    try:
                        self.coulombicPotential[ca,cb] = coulombicPotential[a,b]
                    except:
                        self.coulombicPotential[ca,cb] = 99
                    cb+=1
                ca+=1
            #if fullA[index] == self.atomLabelA[a] and fullB[index] == self.atomLabelB[b]:
            #    self.sepDistance[a*b]=sepDistance[index]
            #    self.lennardJones[a*b]=lennardJones[index]
            #    self.coulombicPotential[a*b]=coulombicPotential[index]
            self.atomNumberA = range(0,len(self.atomLabelA))
            self.atomNumberB = range(0,len(self.atomLabelB))
            #self.sepDistance = np.resize(self.sepDistance,len(self.atomLabelA)*len(self.atomLabelB))
            #self.lennardJones = np.resize(self.lennardJones,len(self.atomLabelA)*len(self.atomLabelB))
            #self.coulombicPotential = np.resize(self.coulombicPotential,len(self.atomLabelA)*len(self.atomLabelB))
        end_time = time.time()
        elapsed = end_time - start_time
        self.output(str(len(self.atomLabelA))+" "+str(len(self.atomLabelB)))
        self.output(str(len(self.lennardJones)))
        self.output("Done. " +str(elapsed)+ "s elapsed",self.RELEASE)
    ...
    def generateAnalysisWidget(self):
        ...
        return new_analysis
#    
#Python code for inputUtil - not used - included as a reference python program
#
    def pyinputUtil(self):
        resfile = []
        ljfile = []
        cfile = []
        nfile = []
        ...
        for prot in prota:
            for res in resfile:
                if prot["resna"]==res["resna"] and prot["atomna"]==res["atomna"] and prot["card"] == "ATOM  ":
...
#        TO DO - error handling for fortran file
        
#        total = input.inpututil(locationAID,locationBID)

        it.inpututil(locationAID,locationBID)
...
#       QMessageBox.information(self,"Large Mapping Detected","Large protein mapping detected. The mapping may take several minutes, during which time the program may appear to hang.",1)
    ...
    def saveResults(self, name):
        ...
        shutil.copyfile("."+os.sep+"finedata.txt",self.ui.batch_output_folder.text()+os.sep+name+"-finedata.txt")
        shutil.copyfile("."+os.sep+"coarsedata.txt",self.ui.batch_output_folder.text()+os.sep+name+"-coarsedata.txt")
        shutil.copyfile("."+os.sep+"fineab.pdb",self.ui.batch_output_folder.text()+os.sep+name+"-fineab.pdb")
        shutil.copyfile("."+os.sep+"coarseab.pdb",self.ui.batch_output_folder.text()+os.sep+name+"-coarseab.pdb")

        shutil.copyfile("."+os.sep+"fort.16",self.ui.batch_output_folder.text()+os.sep+name+".pcr")
...
def cleanup():
    ...
    if os.path.isfile("."+os.sep+"fort.16"):
        os.remove("."+os.sep+"fort.16")
    if os.path.isfile("."+os.sep+"fort.18"):
        os.remove("."+os.sep+"fort.18")
    if os.path.isfile("."+os.sep+"finedata.txt"):
        os.remove("."+os.sep+"finedata.txt")
    if os.path.isfile("."+os.sep+"coarsedata.txt"):
        os.remove("."+os.sep+"coarsedata.txt")        
    if os.path.isfile("."+os.sep+"fineab.pdb"):
        os.remove("."+os.sep+"fineab.pdb")       
    if os.path.isfile("."+os.sep+"finea.pdb"):
        os.remove("."+os.sep+"finea.pdb")
    if os.path.isfile("."+os.sep+"fineb.pdb"):
        os.remove("."+os.sep+"fineb.pdb")       
    if os.path.isfile("."+os.sep+"coarseab.pdb"):
        os.remove("."+os.sep+"coarseab.pdb")        
    if os.path.isfile("."+os.sep+"coarsea.pdb"):
        os.remove("."+os.sep+"coarsea.pdb")        
    if os.path.isfile("."+os.sep+"coarseb.pdb"):
        os.remove("."+os.sep+"coarseb.pdb")        
